			...
			<div class="col-md-6">
				<div class="widget">
					<div class="widget-content-white glossed padded">
						...
						<h3 class="form-title form-title-first"><i class="fa fa-terminal"></i>Update Ad unit <a href="{{urlFor('admin_advertise_delete', {'id' : unit.id})}}" class="btn btn-danger delete delete-btn pull-right"><i class="fa fa-trash-o"></i></a></h3>

					    <form class="create_form" action="{{urlFor('admin_advertise_update', {'id' : unit.id})}}" method="POST">

							<div class="form-group{% if errors.has('type') %} has-error{% endif %}">
								<label class="control-label" for="type">Ad unit type : </label>
								<select name="type" id="type" class="form-control">
									<option value="">Select an ad unit type</option>
									<option value="code" {% if (request.post('type') == "code") or (unit.type == "code") %}selected{%endif%}>Ad code (Google or other)</option>
									<option value="image" {% if (request.post('type') == "image") or (unit.type == "image") %}selected{%endif%}>Image</option>
								</select>
								{% if errors.has('type') %} 
							...
							<div class="form-group{% if (errors.has('size')) or (uniterror is not null) %} has-error{% endif %}">
								...
								<select name="size" id="size" class="form-control">
									...
									{% for size in adsizes %}
										<option value="{{size.id}}" {% if (request.post('size') == size.id) or (unit.adsize_id == size.id) %}selected {%endif%} data-awidth="{{size.width}}" data-aheight="{{size.height}}">{{size.name}}</option>
									{% endfor %}
							...
							<div class="form-group{% if errors.has('code') %} has-error{% endif %} code" style="display:none">
								...
								<textarea name="code" class="form-control" cols="0" rows="0" style="width:100%; height:200px">{% if (request.post('code')) %}{{request.post('code')}}{%else%} {{unit.getCode()|raw}} {%endif%}</textarea>
							...
							<div class="form-group{% if errors.has('url') %} has-error{% endif %} image" style="display:none">
								<div class="form-group{% if errors.has('url') %} has-error{% endif %}">
									...
									<input type="text" name="url" class="form-control" value="{% if (request.post('url')) %}{{request.post('url')}}{%else%}{{unit.url}}{%endif%}">
				  		...
				  		<div class="deals_source_image" id="crop-ad-image">
  							<div class="source_logo avatar-view" title="upload ad image">
  								...
  								{% if adimg is defined %}
	  								...
	  								{% else %}
	  									{% set tmp = false%}	
	  									<img src="{{unit.getImage(baseUrl, images, true)}}">
	  								{% endif %}
...
{% block footer %}
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#ad-size-input').val($('#size').find('option:selected').val());
